...
const seedTourPackages = async () => {
    try {
        ...
        await TourPackage_1.TourPackage.deleteMany({});
        console.log('Cleared existing tour packages');
        const adminUser = await User_1.default.findOne({ role: { $in: ['admin', 'super-admin', 'website-editor'] } });
        let creatorId;
        if (!adminUser) {
        ...
        await TourPackage_1.TourPackage.insertMany(tourPackagesWithCreator);
        console.log('Successfully seeded tour packages');
        const count = await TourPackage_1.TourPackage.countDocuments();
